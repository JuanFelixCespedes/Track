...
class Trips_model extends CI_Model{
	...
	public function getall_trips($trackingcode=false) { 
		...
		if(!empty($tripdata)) {
			foreach ($tripdata as $key => $tripdataval) {
				...
				$newtripdata[$key]['t_user_details'] =  $this->db->select('*')->from('login')->where('u_id',$tripdataval['t_created_by'])->get()->row();
	...
	public function getall_trips2($trackingcode=false) { 
		$newtripdata = array();
		if($trackingcode) {
			$tripdata = $this->db->select('*')->from('trips')->where('t_trackingcode',$trackingcode)->order_by('t_id','desc')->get()->result_array();
		} else {
			$tripdata = $this->db->select('*')->from('trips')->where('t_created_by', $this->session->userdata['session_data']['u_id'])->order_by('t_id','desc')->get()->result_array();
		}
		if(!empty($tripdata)) {
			foreach ($tripdata as $key => $tripdataval) {
				$newtripdata[$key] = $tripdataval;
